...
# Uncomment this code if you're not using it in Power BI  in order to load the previous imputed Titanic dataset in script 01
dataset = pd.read_csv(os.path.join(main_path, "titanic-imputed.csv"))
dataset

# Let's split the dataframe in a small part to be kept for test purpose and a large part for training.
...
settings = {
    ...
    "estimator_list": ["lgbm", "rf", "catboost"],  # Specify models that support callback or don't use it
}

# Get a Pandas series from the single column y_train datarame, as automl.fit requires a series for its y_train parameter
y_train_series = y_train.squeeze()

automl.fit(X_train, y_train_series, **settings)
...
with open(os.path.join(main_path, "titanic-model-flaml.pkl"), 'wb') as f:
    pickle.dump(automl, f, pickle.HIGHEST_PROTOCOL)

# Save the Scikit-learn best estimator in a pkl file for future reuse
with open(os.path.join(main_path, "titanic-best-model-flaml.pkl"), 'wb') as f:
    pickle.dump(automl.model.estimator, f, pickle.HIGHEST_PROTOCOL)

# Merge the feature test dataframe and target test column in a unique test dataframe and save it in a CSV file for future reuse
...
df_test.to_csv(os.path.join(main_path, "titanic-test.csv"), index=False)
...
predictions['prediction_proba'] = y_pred_proba
predictions
