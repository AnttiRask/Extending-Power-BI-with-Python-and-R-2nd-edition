...
# Uncomment this code if you're not using it in Power BI in order to load the imputed Titanic dataset
dataset         <- read_csv(file.path(project_folder, 'titanic-imputed.csv'))
titanic_tbl     <- dataset %>% mutate(Survived = as.factor(Survived))
...
rf_grid <- crossing(mtry = 2:4,min_n = seq(10,40,2))
rf_tune <- tune_grid(
    titanic_wf %>% add_model(rf_spec),
...
# Uncomment this code if you're not using it in Power BI to get metric values
...
# Save test dataset
# Warning! This will replace the file you created using Python.
titanic_testing %>% 
