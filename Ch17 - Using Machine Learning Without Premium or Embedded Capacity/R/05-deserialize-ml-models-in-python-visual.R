...
getModelFromStringSerializedDataframe <- function(str_serialized_df) {
    ...
    # Sort the contents of the dataframe by model_id and chunk_id, to ensure that the string chunks are sorted correctly and ready to be deserialized
...
prediction_label <- predict(
    selected_model, 
    new_data = input_tuple_df, 
    type     = 'class'
) %>%
