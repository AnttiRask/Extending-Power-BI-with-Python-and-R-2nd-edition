...
deduplicate_data <- function(
    ...
    deduplicated_data           <- tibble(!!!setNames(rep(list(NULL), length(new_columns)), new_columns))
    duplicated_data             <- tibble(!!!setNames(rep(list(NULL), length(new_columns)), new_columns))
...
# Usage example
data                      <- fromJSON("C:/R/Extending-Power-BI-with-Python-and-R-2nd-edition/Ch12 - Calculating Columns Using Complex Algorithms, Distances/email-addresses-to-dedup.json")

deduplicated_data_jaccard <- deduplicate_data(
    data,
...
deduplicated_data_jaro_winkler <- deduplicate_data(
    data,
    similarity_method    = "jw",
    p                    = 0.1,
    similarity_threshold = 0.872
    )
deduplicated_data_jaro_winkler[[1]] # deduplicated_data
deduplicated_data_jaro_winkler[[2]] # duplicated_data

deduplicated_data_jaccard_3grams <- deduplicate_data(
    data,
    similarity_method    = "jaccard",
    q                    = 3,
...
deduplicated_data_jaro_winkler_3grams <- deduplicate_data(
    data,
    similarity_method    = "qgram",
    q                    = 3,
