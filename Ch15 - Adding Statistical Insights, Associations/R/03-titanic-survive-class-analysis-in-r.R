...
violinPlot <- function(data, varx, vary, title, xlab, ylab, alpha = 0.5, jitter = FALSE) {
...
calc_corr <- function(data, row_name, col_name, numeric_replace_value = 0, theil_uncert = TRUE) {
    ...
    if (row_data_type == 'numeric' & col_data_type == 'numeric') {
        ...
        return((c %>% correlate(method = 'pearson', quiet = TRUE))[[1,3]])
    }
    ...
    if (theil_uncert) {
        return(UncertCoef(row_vec, col_vec, direction = 'row'))
    } else {
        return(cramer_v(x = row_vec, y = col_vec))
    }
}
...
cramer_v(x = tbl$Survived, y = tbl$Pclass)

# Let's verify that Cramér's V is a symmetric function
cramer_v(x = tbl$Survived, y = tbl$Pclass) == cramer_v(x = tbl$Pclass, y = tbl$Survived)

# You can also draw a mosaic plot for these variables
mosaic(
    ~ Survived + Pclass,
    data  = tbl,
    main  = "Survived VS Passenger class", 
    shade = TRUE
)
...
UncertCoef(tbl$Survived, tbl$Pclass, direction = 'row')

# Just check that the opposite direction gives you a different result
UncertCoef(tbl$Pclass, tbl$Survived, direction = 'row')
...
# the numeric one Age
correlation_ratio(
    categories            = tbl$Pclass,
    measurements          = tbl$Age,
...
ass <- crossing(row, col)

# Add the corr column containing correlation values (takes about twenty seconds)
corr_tbl <- ass %>%
    mutate(
        corr = map2_dbl(
            row,
