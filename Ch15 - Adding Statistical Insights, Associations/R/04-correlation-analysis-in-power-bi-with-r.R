...
calc_corr <- function(data, row_name, col_name, numeric_replace_value = 0, theil_uncert = TRUE) {
    ...
    if (row_name == col_name) {
        ...
        corr                    <- (c %>% correlate(method = 'pearson', quiet = TRUE))[[1,3]]
        ...
        corr <- correlation_ratio(
            categories            = col_vec,
            measurements          = row_vec,
        ...
        if (theil_uncert) {
            corr <- UncertCoef(row_vec, col_vec, direction = 'row')
        } else {
            corr <- cramer_v(x=row_vec, y = col_vec)
        }
...
corr_tbl <- ass %>% 
    mutate( corr = map2_dbl(row, col, ~ calc_corr(data = df, row_name = .x, col_name = .y, theil_uncert = TRUE)))
